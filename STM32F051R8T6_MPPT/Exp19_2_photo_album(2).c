...
int main(void)
{
  ...
  TFT_string(0,6,White,Magenta,"           BMP 파일  전자앨범           ");
  TFT_string(0,8,Green,  Black,"****************************************");
  TFT_string(4,14,Cyan,   Black,"320x240 픽셀의 랜드스케이프 화면");
  TFT_string(4,17,White,  Black,"              또는              ");
  TFT_string(4,20,Green,  Black," 240x320 픽셀의 포트레이트 화면 ");
  Delay_ms(3000);
  ...
  while(1)
    { if(display_flag == 1)			// display BMP file, file number, and file name
      ...
      if((play_flag == 1) && (timer >= 5))	// go next file by 5 sec step
	{ timer = 0;
	  if(file_number < (total_file - 1))            
	    file_number++;
	  else
...
void TFT_filename(U08 file_number)		/* display BMP file name */
{
  ...
  if(file_flag == 0)				// short file name(8.3 format)
    TFT_short_filename(8,0,Yellow,Transparent);
  else if(file_flag == 1)			// long file name
    TFT_long_filename(8,0,Yellow,Transparent);
  else if(file_flag == 2)			// file name is longer than 195 characters
    TFT_string(8,0,White,Transparent,"* 파일명 길이 초과 *");
  else						// file name error
    TFT_string(8,0,White,Transparent,"*** 파일명 오류 ***");
