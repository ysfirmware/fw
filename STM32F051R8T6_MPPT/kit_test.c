/* ============================================================================ */
/*	kit_test.c : OK-STM32F0 키트 V1.0 하드웨어 테스트 프로그램		*/
/* ============================================================================ */
...
int main(void)
{
  ...
  TFT_string(6, 6,Cyan,   Black,"    OK-STM32F0 키트 V1.0    ");
  TFT_string(6,10,White,Magenta,"  하드웨어 테스트 프로그램  ");
  TFT_string(6,18,White,  Black,"    2013년 09월 01일(일)    ");
...
void Display_screen(void)			/* display test screen */
{
  ...
  TFT_string( 0, 0,White,Magenta,"   OK-STM32F0 키트 V1.0 진단 프로그램   ");
  TFT_string( 0, 3,Cyan,   Black,"   (1) TFT-LCD 컬러 디스플레이 테스트");
  TFT_string( 0, 6,Green,  Black,"   (2) TFT-LCD 터치스크린 입력 테스트");
  TFT_string( 0, 9,Cyan,   Black,"   (3) MCO 단자 클록 출력 테스트");
...
void Test1(void)				/* Test 1 */
{
  ...
  TFT_string( 0,16,White,  Black," 컨트롤러/드라이버 HX8347-G(대만 Himax) ");
  ...
  TFT_string( 0,24,White,  Black,"  컨트롤러/드라이버 HX8347-G  ");
...
void Test3(void)				/* Test 3 */
{
  ...
  RCC->CSR &= 0xFFFFFFFE;			// LSION = 0
  RCC->CR2 &= 0xFFFFFFFE;			// HSI14ON = 0
  Beep();
}
...
void Test4(void)				/* Test 4 */
{
  ...
  TFT_string(0,5,White,Magenta,"        A/D 컨버터  입력 테스트         ");
  TFT_string(9, 9,White,Black,"ADC0 (+5.0V) = +0.00V");
  TFT_string(9,12,White,Black,"ADC1 (+3.3V) = +0.00V");
  TFT_string(9,15,White,Black,"ADC2 ( VR1 ) = +0.00V");
  TFT_string(9,18,White,Black,"ADC3(LM35DZ) = +00.0 C");
  TFT_string(9,21,White,Black,"ADC17(+1.2V) = +0.00V");
  TFT_xy(29,18);
  TFT_English(0x7F);
  ...
  ADC->CCR = 0x00400000;			// enable VREFINT
  ...
  while(1)
    { ADC1->CHSELR = 0x00000001;		// channel 0 (+5.0V)
      ...
      result >>= 4;				// calculate average
      TFT_xy(24,18);				// display temperature
...
void Test5(void)				/* Test 5 */
{
  ...
  TFT_string( 4,18,Cyan, Black,"약 500Hz의 삼각파 출력 (0~+3.3V)");
  ...
  TFT_string(11,15,White,Black,"TP3(DAC1_OUT) 단자");
  TFT_string( 4,18,Cyan, Black,"약 500Hz의 정현파 출력 (0~+3.3V)");
  Beep();
...
void Test6(void)				/* Test 6 */
{
  ...
  unsigned char string[10] = { "OK-STM32F0" };
  ...
  TFT_string(9,18,Cyan, Black,"송신 문자 : ");
  TFT_string(9,21,Cyan, Black,"수신 문자 : ");
  Delay_ms(1000);
  ...
  for(i = 0; i < 10; i++)
    { USART1->TDR = string[i];			// transmit test data
      TFT_xy(21+i,18);				// display transmitted test data
      TFT_color(Green,Black);
      TFT_English(string[i]);
      Delay_ms(10);

      RXD = USART1->RDR;			// receive test data
      TFT_xy(21+i,21);				// display transmitted test data
      TFT_color(Magenta,Black);
      TFT_English(RXD);
...
void Test7(void)				/* Test 7 */
{
  ...
  TFT_string(10,12,White,  Black,"0000년 00월 00일(월)");
  TFT_string(13,16,White,  Black,"오전 00:00:00");
...
void DS3234_initialize(void)			/* initialize DS3234 RTC */
{
  ...
  if((second > 0x59) || (minute > 0x59) || (hour < 0x40) || (weekday > 0x07) || (date > 0x31) || (month > 0x12) || (year > 0x99))
    { DS3234_write(0x00, 0x00);			// second = 0x00
      ...
      DS3234_write(0x03, 0x03);			// weekday = 0x03(Tuesday)
      DS3234_write(0x04, 0x15);			// date = 0x15
...
void Test8(void)				/* Test 8 */
{
  ...
  TFT_string(13,37, Silver,Black, "00:00:00");
