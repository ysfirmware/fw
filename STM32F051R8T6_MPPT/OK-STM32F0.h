...
//	(3) 최적화 옵션  : High/Speed
//	(4) CSTACK 크기  : 0x1000
//      (5) TFT-LCD 모듈 : I3202-7HMT2432A V2.0
// -------------------------------------------------------------------------------
...
void Initialize_TFT_LCD(void)			/* initialize TFT-LCD with HX8347-G */
{
  ...
  TFT_write(0x2E, 0x0088);			// GDOFF 
  TFT_write(0x29, 0x008F);			// RTN 
  TFT_write(0x2B, 0x0004);			// DUM 
  TFT_write(0xE4, 0x0010);			// EQ_S1 
  TFT_write(0xE5, 0x0010);			// EQ_S2 
  TFT_write(0xE6, 0x0010);			// EQ_S3 
  TFT_write(0xE7, 0x0010);			// EQ_S4 
  TFT_write(0xEA, 0x0000);			// PTBA[15:8] 
  TFT_write(0xEB, 0x0000);			// PTBA[7:0] 
  TFT_write(0xEC, 0x003C);			// STBA[15:8] 
  TFT_write(0xED, 0x00C7);			// STBA[7:0] 
  TFT_write(0xE8, 0x0070);			// OPON[7:0] 

  TFT_write(0x40, 0x0001);			// Gamma 2.2 Setting 
  TFT_write(0x41, 0x0007);
  TFT_write(0x42, 0x0007);
  TFT_write(0x43, 0x001E);
  TFT_write(0x44, 0x001C);
  TFT_write(0x45, 0x003E);
  TFT_write(0x46, 0x001B);
  TFT_write(0x47, 0x006B);
  TFT_write(0x48, 0x0007);
  TFT_write(0x49, 0x0013);
  TFT_write(0x4A, 0x001A);
  TFT_write(0x4B, 0x0019);
  TFT_write(0x4C, 0x0016);
  TFT_write(0x50, 0x0001);
  TFT_write(0x51, 0x0023);
  TFT_write(0x52, 0x0021);
  TFT_write(0x53, 0x0038);
  TFT_write(0x54, 0x0038);
  TFT_write(0x55, 0x003E);
  TFT_write(0x56, 0x0014);
  TFT_write(0x57, 0x0064);
  TFT_write(0x58, 0x0009);
  TFT_write(0x59, 0x0006);
  TFT_write(0x5A, 0x0005);
  TFT_write(0x5B, 0x0006);
  TFT_write(0x5C, 0x0018);
  TFT_write(0x5D, 0x00CC);

  TFT_write(0x1B, 0x001A);			// VRH=4.60V 
  TFT_write(0x1A, 0x0001);
  TFT_write(0x24, 0x0070);			// VMH 
  TFT_write(0x25, 0x0058);			// VML 
  TFT_write(0x23, 0x0078);			// VMF

  TFT_write(0x18, 0x0036);			// Power on Setting
  TFT_write(0x19, 0x0001);
  TFT_write(0x01, 0x0000);
  TFT_write(0x1F, 0x0088);
  Delay_ms(5); 
  TFT_write(0x1F, 0x0080);
  Delay_ms(5); 
  TFT_write(0x1F, 0x0090);
  Delay_ms(5); 
  TFT_write(0x1F, 0x00D4);
  Delay_ms(5); 
  TFT_write(0x17, 0x0005);			// 16-bit/pixel = 65536 colors

  TFT_write(0x36, 0x0008);			// SET PANEL (BGR_PANEL = 0)
  TFT_write(0x28, 0x0038);			// Display ON Setting
  Delay_ms(40); 
  TFT_write(0x28, 0x003C);
