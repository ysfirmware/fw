 ...
 Vout_m_avg = V_gain *(float)result*3.3/4096.;
  ...
  //------------ MPPT Algorit m------------
    ...
    if(P_diff>0){
      ...
      if(V_diff>0)      duty = duty-10;
      else if(V_diff<0) duty = duty+10;
...
int main(void)
{
 ...
 Vout_m_avg = V_gain *(float)result*3.3/4096.;
  ...
  P = Vout_m_avg*Iout_m_avg;
    ...
    if((key == KEY3) && (duty != 4750)){		// if KEY3, increment duty ratio by 10%
      duty += 50;
      if(duty>4799) duty=4799;
    }
    else if((key == KEY4) && (duty != 0)){	// if KEY4, decrement duty ratio by 10%
      duty -= 50;
      if(duty<0) duty=0;
    }
    ...
    /*
    TFT_xy(25,25);				// display temperature
    TFT_color(White,Black);
    TFT_unsigned_float((float)duty/4800*100., 3, 1);*/
  }
}
