...
unsigned char cursor, year, month, date, hour, minute, second;
unsigned char old_second, blink;
...
void DSP_RTC(void)                              /* display clock from RTC */
...
void DSP_LED(void)                              /* display clock on LED */
{ unsigned char i;
...
int main(void)
{
  ...
  LCD_string(0x80," 2006/10/01 SUN ");          // display title
  ...
  if(RTC_FLAG != 'G')				// RTC data are available ?
    { RTC_FLAG = 'G';                           // initialize RTC data flag
      ...
      RTC_MONTH = 0x10;                         // initialize month(10)
