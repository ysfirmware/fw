;[][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][]
;[]								   []
;[]   Exp10_2.asm : A/D Converter ADC3 Input by Free Running Mode  []
;[]								   []
;[][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][]
;
...
;------------------------------------------------
	...
	LDI	AL,0b10100101		; ADC enable, 500kHz
	STS	ADCSRA,AL
	LDI	AL,0b00000000
	STS	ADCSRB,AL
	...
	CALL	D50US
	CALL	D100US
	LDI	AL,0b11100101		; start conversion with free running mode
	STS	ADCSRA,AL
	CALL	D1MS
...
LOOP:	LDI	CL,16			; read ADC by 16 times
	CLR	BH			; clear sum
	CLR	BL
LOOP1:	LDS	AL,ADCL			; ADCL must be read first, then ADCH
	LDS	AH,ADCH
