;[][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][]
;[]								   []
;[]   Exp16_2.asm : Digital Clock on LCD Module and 7-Segment LED  []
;[]								   []
;[][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][][]
;
...
.def	OLD_SEC	= R9			; old second
.def	BLINK	= R10			; blink count
...
DSP_RTC:LDI	LCD_BUFFER,0x0C		; cursor OFF
	...
	CPI	BL,0x00
	BREQ	DSP_RTC1
	CALL	DSP_LED			; display clock on 7-segment LED
	RET
...
;------------------------------------------------
DSP_LED:LDS	AH,RTC_SECOND		; read second from RTC
	CP	AH,OLD_SEC		; new second ?
	BREQ	DSP_LED1
	CLR	BLINK			; if yes, clear blink counter
	MOV	OLD_SEC,AH
DSP_LED1:INC	BLINK			; increment blink counter

	MOV	AL,AH			; if 11~15, 31~35 or 51~55 second,
	ANDI	AL,0b00011111		;    then display date
	CPI	AL,0x11
	BRNE	DSP_LED2
	JMP	LED_YEAR
DSP_LED2:CPI	AL,0x12
	BRNE	DSP_LED3
	JMP	LED_YEAR
DSP_LED3:CPI	AL,0x13
	BRNE	DSP_LED4
	JMP	LED_YEAR
DSP_LED4:CPI	AL,0x14
	BRNE	DSP_LED5
	JMP	LED_YEAR
DSP_LED5:CPI	AL,0x15
	BRNE	DSP_LED6
	JMP	LED_YEAR

DSP_LED6:MOV	AL,AH			; display second
	LSR	AL
	LSR	AL
	LSR	AL
	LSR	AL
	...
	STS	DIG_SELECT,AL
	CALL	D1MS
	LDS	AH,RTC_MINUTE		; read minute from RTC
	MOV	AL,AH			; display minute
	LSR	AL
	LSR	AL
	LSR	AL
	LSR	AL
	...
	LDS	AH,RTC_HOUR		; read hour from RTC
	ANDI	AH,0b01111111
	MOV	AL,AH			; display hour
	LSR	AL
	LSR	AL
	LSR	AL
	LSR	AL
	...
	STS	DIG_SELECT,AL
	CALL	D1MS
	MOV	BL,BLINK		; 0.5 sec over ?
	CPI	BL,60
	BRSH	DSP_LED7
	LDI	AH,0b00000010		; if no, display "-"
	JMP	DSP_LED8
DSP_LED7:LDI	AH,0b00000000		; if yes, display " "
DSP_LED8:STS	LCD_DATABUS,AH
	LDI	AL,0b00100000
	STS	DIG_SELECT,AL
	CALL	D1MS
	STS	LCD_DATABUS,AH
...
LED_YEAR:LDI	AH,0x20			; upper 2 digits of year
	MOV	AL,AH
	LSR	AL
	LSR	AL
	LSR	AL
	...
	STS	DIG_SELECT,AL
	CALL	D1MS
	LDS	AH,RTC_YEAR		; read year from RTC
	MOV	AL,AH			; display year
	LSR	AL
	LSR	AL
	LSR	AL
	LSR	AL
	...
	BRSH	LED_Y1
	ORI	AL,0b00000001
LED_Y1:	STS	LCD_DATABUS,AL
	LDI	AL,0b00010000
	STS	DIG_SELECT,AL
	CALL	D1MS
	LDS	AH,RTC_MONTH		; read month from RTC
	MOV	AL,AH			; display month
	LSR	AL
	LSR	AL
	LSR	AL
	LSR	AL
	...
	BRSH	LED_Y2
	ORI	AL,0b00000001
LED_Y2:	STS	LCD_DATABUS,AL
	LDI	AL,0b00000100
	STS	DIG_SELECT,AL
	CALL	D1MS
	LDS	AH,RTC_DATE		; read date from RTC
	MOV	AL,AH			; display date
	LSR	AL
	LSR	AL
	LSR	AL
	LSR	AL
