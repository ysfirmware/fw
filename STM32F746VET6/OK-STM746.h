...
void Initialize_MCU(void)			/* initialize STM32F746VET6 MCU */
{
// (1) 데이터 캐시 및 명령 캐시 설정		// 최적화를 size로 설정해야 시스템이 안정적으로 동작.
						// 최적화를 speed로 하면 데이터 캐시를 사용하지 말것.
#if (CACHE_FLAG == 1)
...
// (8) 키트에 맞게 GPIO를 초기화
  ...
  GPIOE->ODR = 0x0000004F;			// -TFT_RESET = -TFT_CS = TFT_RS = -TFT_WR = 1
...
void Initialize_LCD(void)			/* initialize text LCD module */
{
  RCC->AHB1ENR |= 0x00000028;			// enable clock of port D,E
...
void Initialize_TFT_LCD(void)			/* initialize TFT-LCD with HX8347-A */
{
  ...
  GPIOE->BSRR = 0x00080000;			// TFT-LCD reset
  Delay_ms(1);
