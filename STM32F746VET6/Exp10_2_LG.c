...
#define ARR_value	16200			// 120% of Leader_Code
...
volatile unsigned char Remocon_state = 0;	// step to recognize remocon signal
volatile unsigned int Remocon_count= 0;		// 1us base time counter
volatile unsigned char Remocon_data= 0;		// temporary byte data of remocon signal
volatile unsigned char Remocon_bit_count= 0;	// bit count of data to recognize remocon signal
volatile unsigned char Remocon_OK_flag= 0;	// remocon command receive OK
volatile unsigned char Remocon_command[4] ={0.};// remocon command string
...
int main(void)
{
  ...
  TIM6->PSC = 107;				// 108/(107+1) = 1MHz = 1us base time
